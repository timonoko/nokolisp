...
(defq BOOT
 (any-key select-disk error-hook _vtcolor with-definition-of %tracep%
  * *+-/ *main-loop* *mapgen* *setf-method-1* *struct* +
  - / : < <=> = > BOOT END-OF-FILE IBM-t”rky„ LSEEK Mapc
  _vt_coma ` `2 append append-to-file autost break caaaar
  caaadr caaar caadar caaddr caadr cadaar cadadr cadar caddar
  case cd cdaaar cdaadr cdaar cdadar cdaddr cdadr cdar
  cddaar cddadr cddar cdddar cddddr char chdir command-line comment
  compile compile-all copy date dd definition-of defmacro defstruct
  defun del-member delete delete-file delete-line dir
  dispose divide do dolist dos-eval dotimes edit edit-line edit3
  einsert eprint eprint2 erase_page esc fast-spawn fib
  fil-exists file-size filter flat flet for gensym get
  get-cur-dir get-cur-disk home insert-line intersection let
  let* load locate macroexpand make-exe map mapc mapcan mapcar
  mapcon maplist mergesort mkdir msdos nmerge nmerge1 nth
  nthdel nthpush nthswap orderp plm-item pop ppr-def pr-many
  princ print-struct print-to-file print-to-list prints push
  push-once put quit read-from-file read-from-list read-with-edit
  readcc-Ž-™ rmdir save set_cursor setf setf-method sort
  sp spawn split string-append subst synon-for-prims time
  trace uncompile unless unlink untrace unwind-protect when
  where-is-ref))
...
(defun erase_page () (repeat-times (* 24 80) (printc 32)) (home) )

(defq esc (mlambda () '(printc 27)))
...
(defq spawn
 (lambda
  ...
  (unless com-file (setq com-file 'C:\command.com))
  (if
   (fil-exists com-file)
   (progn

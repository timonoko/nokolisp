...
'(%Z%%M% %R%.%L%.%B%.%S% %E% %Y%)
'(MIKKO-3 (1 / 3 - 2003) (16 : 22 : 44 38))
(defq *package* NOKVERTX)

(defun vertex
 (vert)
 (prints 'FACET 'NORMAL 0 0 -1)
 (cr)
 (prints 'OUTER 'LOOP)
 (cr)
 (mapc vert
  (function
   (lambda
    (v)
    (print 'VERTEX)
    (mapc v (function (lambda (v2) (sp) (format 6 v2))))
    (cr))))
 (print 'ENDLOOP)
 (cr)
 (print 'ENDFACET)
...
(defq NOKVERTX
 (vertex aak format sqrt tan cos *FLOAT-OPET* pi arctan sin
  integer make-float-expr NOKVERTX float fib tsr5-from-string tsr5-to-string
  tsr5))
